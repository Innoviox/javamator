...
def main(csid, psid,
    ...
    shutil.copytree(f"submissions/{csid}/{psid}",
                    f"run/src/{directory_prefix}/{csid}/{psid}")
    ...
    if os.path.isdir(f"psets/{psid}/junit"):
        ...
        for junit in junits:
            ...
            shutil.copy(f"psets/{psid}/junit/{junit}",
                        f"run/src/{directory_prefix}/{csid}/{psid}/{junit}")
            # change package name
            junitfile = open(
                f"run/src/{directory_prefix}/{csid}/{psid}/{junit}").read().split("\n")
    ...
    vid = subprocess.check_output(
        ["docker", "volume", "create"]).strip().decode("ascii")
    volpath = f"/var/lib/docker/volumes/{vid}/_data/"
    p = subprocess.Popen(["cp", "-r", "run", volpath])

    # create docker container
    p = subprocess.Popen(["docker", "run", "-td", "-v",
                          f"{vid}:/mnt", "openjdk"], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
    cid, error = p.communicate()
    cid = cid.strip().decode("ascii")
    command_prefix = ["docker", "exec", "-i", "-w", "/mnt/run", cid]

    # compile submission
    p = subprocess.Popen(command_prefix + ["javac", "-d", f"bin", "-cp", "junit.jar:lib/*"] + [path[len(f"{volpath}run/"):] for path in glob.glob(
        f"{volpath}run/src/**/*.java", recursive=True)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if err:
        ...
        p = subprocess.Popen(command_prefix + ["java", "-jar", "junit.jar", "--class-path", f"bin:lib/*", "-p",
                                               f"{package_prefix}.{csid}.{psid}", "--details", "flat", "--details-theme", "ascii"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        ...
        for classname in classes:
            out = b""
            p = subprocess.Popen(command_prefix + ["java", "-cp", "bin:lib/*", f"{package_prefix}.{csid}.{psid}.{classname}"] + (
                classes[classname]["arguments"] if "arguments" in classes[classname] else []), stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            fcntl.fcntl(sys.stdin, fcntl.F_SETFL, fcntl.fcntl(
                sys.stdin, fcntl.F_GETFL) | os.O_NONBLOCK)
            ...
            if classes[classname].get("output") != None:
                if out == classes[classname].get("output"):
